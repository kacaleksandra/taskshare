...
import { AssignmentMiniProps, getAllSubmitions, getAssignmentInfo } from './_api/client';
...
export default function Page({ params }: { params: { id: string } }) {
  ...
  const [assignmentInfo, setAssignmentInfo] = useState<AssignmentMiniProps | null>(null);
  const [submissions, setSubmissions] = useState<[]>([]);

  const { mutate: loadAllSubmisions } = useMutation({
    mutationFn: getAllSubmitions,
    onError: () => {
      toast({ description: 'Failed to load submisions, please try again later.' });
    },
  ...
  const { mutate: loadAssignmentInfo } = useMutation({
    ...
    onError: () => {
      toast({ description: 'Failed to load assignment info, please try again later.' });
    },
    onSuccess: async (response) => {
  ...
  <div className='max-w-full'>
    <div className='max-w-full items-center flex flex-col'>
      ...
      <h3 className='w-4/5 text-left text-2xl m-4 font-bold'>Submissions
      </h3>
      {submissions.map((submision) => (
        JSON.stringify(submision)
